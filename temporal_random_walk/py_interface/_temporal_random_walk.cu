...
PYBIND11_MODULE(_temporal_random_walk, m)
{
    py::class_<TemporalRandomWalk>(m, "TemporalRandomWalk")
        .def(py::init([](
             ...
             cuda_device_id (int, optional): CUDA device index to pin this instance to when use_gpu=True. Lets a multi-GPU process run TemporalRandomWalk on one GPU while another library (e.g. PyTorch) holds a different GPU on the same thread; the device is set on entry to every method and restored on exit. Ignored when use_gpu=False. Defaults to 0.
             )",
             py::arg("is_directed"),
